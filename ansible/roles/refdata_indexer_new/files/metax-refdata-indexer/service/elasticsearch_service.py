...
class ElasticSearchService:
    ...
    def create_type_mapping(self, index, doc_type, filename):
        ...
        return self._operation_ok(self.es.indices.put_mapping(index=index, doc_type=doc_type,
            body=self._get_json_file_as_str(filename)))

    def delete_and_update_indexable_data(self, index, doc_type, indexable_data_list):
        if len(indexable_data_list) > 0:
            bulk_update_str = "\n".join(map(lambda idx_data:
                self._create_bulk_update_row_for_indexable_data(index, doc_type, idx_data), indexable_data_list))
    ...
    def _delete_all_documents_from_index_with_type(self, index, doc_type):
        ...
        return self._operation_ok(self.es.delete_by_query(index=index, doc_type=doc_type,
            body="{\"query\": { \"match_all\": {}}}"))

    def _create_bulk_update_row_for_indexable_data(self, index, doc_type, indexable_data_item):
        return "{\"index\":{\"_index\": \"" + index + "\", \"_type\": \"" + doc_type + "\",\"_id\":\"" + \
    ...
    def _create_bulk_delete_row_indexable_data(self, index, doc_type, indexable_data_item):
        return "{\"delete\":{\"_index\": \"" + index + "\", \"_type\": \"" + doc_type + "\", \"_id\":\"" + \
            indexable_data_item.get_es_document_id() + "\"}}"
