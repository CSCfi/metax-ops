...
def main():
    ...
    instructions = instructions.format(
        indices=str([NO, ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME,
                ElasticSearchService.ORGANIZATION_DATA_INDEX_NAME]),
        types=str([NO, ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME, IndexableData.DATA_TYPE_ORGANIZATION] +
    ...
    if REMOVE_AND_RECREATE_INDEX in run_args:
        ...
        if remove_and_recreate_index not in [NO, ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME,
                ElasticSearchService.ORGANIZATION_DATA_INDEX_NAME]:
            _logger.error(instructions)
            sys.exit(1)
    ...
    if types_to_reindex in ([ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME,
            ReferenceData.DATA_TYPE_RESEARCH_INFRA]):
    ...
    if types_to_reindex in [ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME]:
        for data_type in ReferenceData.FINTO_REFERENCE_DATA_TYPES:
            ...
            es.delete_and_update_indexable_data(ElasticSearchService.REFERENCE_DATA_INDEX_NAME, data_type,
                    finto_es_data_models)
        ...
        if len(finto_es_data_models) > 0:
            es.delete_and_update_indexable_data(ElasticSearchService.REFERENCE_DATA_INDEX_NAME, types_to_reindex,
                    finto_es_data_models)
        else:
    ...
    if types_to_reindex in [ALL, IndexableData.DATA_TYPE_ORGANIZATION]:
        es.delete_and_update_indexable_data(ElasticSearchService.ORGANIZATION_DATA_INDEX_NAME,
                IndexableData.DATA_TYPE_ORGANIZATION, org_service.get_data())
    ...
    if types_to_reindex in [ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME,
        ...
        if len(infra_es_data_models) > 0:
            es.delete_and_update_indexable_data(ElasticSearchService.REFERENCE_DATA_INDEX_NAME,
                    ReferenceData.DATA_TYPE_RESEARCH_INFRA, infra_es_data_models)
        else:
    ...
    if types_to_reindex in [ALL, ElasticSearchService.REFERENCE_DATA_INDEX_NAME, ReferenceData.DATA_TYPE_MIME_TYPE]:
        ...
        if len(mime_es_data_models) > 0:
            es.delete_and_update_indexable_data(ElasticSearchService.REFERENCE_DATA_INDEX_NAME,
                    ReferenceData.DATA_TYPE_MIME_TYPE, mime_es_data_models)
        else:
